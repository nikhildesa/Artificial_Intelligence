...
class ExactInference(Inference):
        ...
        for r in range(self.n_rows):
            for c in range(self.n_cols):
                ...
                for i in range(len(landmarks)):
                    posterior_prob = self.belief[r][c]
                    distSQ = math.sqrt((landmarks[i][0] - x)**2 + (landmarks[i][1] - y)**2)
                    pdf.append(normal_pdf(distSQ,App.SENSOR_NOISE,observed_distances[i]))
                new_prob = np.prod(pdf)
                self.belief[r][c] = posterior_prob * new_prob
        self.belief = self.normalize(self.belief)
    ...
    def timeUpdate(self):
        ...
        for r in range(self.n_rows):
            for c in range(self.n_cols):
                posterior_prob = self.belief[r][c]
                transistion = self.transition_model(r,c)
                for t in transistion:
                    new_prob = t[0]
                    r_,c_ = t[1]
                    self.belief[r_][c_]+=posterior_prob * new_prob
        self.belief = self.normalize(self.belief)
...
class ParticleFilter(Inference):
    ...
    def observe(self,
        ...
        for i in range(self.NUM_PARTICLES):
            x,y = self.get_coordinate(self.particles[i][0],self.particles[i][1])
            pdf = []
            for j in range(len(landmarks)):
                distSQ = math.sqrt((landmarks[j][0] - x)**2 + (landmarks[j][1] - y)**2)
                pdf.append(normal_pdf(distSQ,App.SENSOR_NOISE,observed_distances[j]))
            new_prod = np.prod(pdf)
            weights[i] = new_prod
        storage = self.particles.copy()
        for i in range(self.NUM_PARTICLES):
             newParticle = random.choices(storage,weights = weights,k = 1)
             self.particles[i] = newParticle[0]
        self.updateBelief()
    ...
    def timeUpdate(self):
        ...
        for i in range(self.NUM_PARTICLES):
            transistion = self.transition_model(self.particles[i][0],self.particles[i][1])
            weights = [0] * len(transistion)
            for j in range(len(weights)):
                prob = transistion[j][0]
                weights[j] = prob
            newParticle = random.choices(transistion,weights,k = 1)
            location = newParticle[0][1]   
