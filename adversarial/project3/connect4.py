...
###############################################################################
    def value(player, board, depth_limit):
        if depth_limit == 0 or board.terminal() == True:  # game finished
            score = evaluate(max_player,board)            # return v
            return score,None

        else:
            
            if player == max_player:  # max player
                v,placement = max_value(max_player,board,depth_limit)   # go to max
                return v,placement
            
            else:                     # min player
                v,placement = min_value(next_player,board,depth_limit)  # go to min
                return v,placement
    ...
    def min_value(player, board, depth_limit):
        ...
        for child in all_childs:
            ...
            new_value,new_placement = value(max_player,new_board,depth_limit - 1)
            if new_value < v:
                v = new_value
                placement = column
...
###############################################################################
    def value(player, board, depth_limit,alpha,beta):
        ...
        if depth_limit == 0 or board.terminal() == True:
            score = evaluate(max_player,board)
            return score,None
        
        else:    
            
            if player == max_player:
                v,placement = max_value(max_player,board,depth_limit,alpha,beta)
                return v,placement
            
            else:
                v,placement = min_value(next_player,board,depth_limit,alpha,beta)
                return v,placement
    ...
    def min_value(player, board, depth_limit,alpha,beta):
        ...
        for child in all_childs:
            ...
            new_value,new_placement = value(max_player,new_board,depth_limit - 1,alpha,beta)
            if new_value < v:
                v = new_value
                placement = column
            if new_value<beta:
...
###############################################################################
    def value(player, board, depth_limit):
        if depth_limit == 0 or board.terminal() == True:
            ...
            if player == max_player:
                v,placement = max_value(max_player, board, depth_limit)
                return v,placement
    ...
    def min_value(player, board, depth_limit):
        ...
        v = mean(values)
        return v,None    # we dont care about placement it is chose uniformily at random. Read 338.
