        ...
        for child in valid_children:
            if (child not in open_set and child not in closed_set):
                    if child == goal: 
                        ...
                        parent_tree.append(goal)
                        while(goal!=start):
                            goal_parent = parent[goal[0]][goal[1]]
                            parent_tree.append(goal_parent)
                            goal = goal_parent
                        parent_tree = parent_tree[::-1]
                        for i in range(1,len(parent_tree)):
                            movement.append((parent_tree[i][0] - parent_tree[i - 1][0],parent_tree[i][1] - parent_tree[i - 1][1]))
...
def uniform_cost_search(grid_size, start, goal, obstacles, costFn, logger):
    ...
    while(len(open_set) > 0):
        node,parent_gvalue = open_set.pop()
        if node == goal:
            closed_set.add(node)   
            parent_tree.append(goal)
            while(goal!=start):
        ...
        for child in valid_children:
            if (child not in open_set and child not in closed_set):
                g = parent_gvalue + costFn(child) 
                open_set.put((child[0],child[1]),g)
                parent[child[0]][child[1]] = (node[0],node[1])
                
            elif (child in open_set and g < costFn(child)):
                open_set.put((child[0],child[1]),g)
                parent[child[0]][child[1]] = (node[0],node[1])
...
#############################################################################
    ...
    while(len(open_set) > 0):
        ...
        if node == goal:
            ...
            for child in valid_children:
                g = parent_gvalue[1] + costFn(child)
                f = g + heuristic(child[0],child[1])
                if (child not in open_set and child not in closed_set): 
                    open_set.put((child[0],child[1]),(f,g))
                    parent[child[0]][child[1]] = (node[0],node[1])
                    
                elif (child in open_set and f < open_set.get(child)[0]):
                    open_set.put((child[0],child[1]),(f,g))
                    parent[child[0]][child[1]] = (node[0],node[1])
